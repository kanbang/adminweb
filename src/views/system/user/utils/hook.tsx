...
export function useUser(tableRef: Ref) {
  ...
  async function onSearch() {
    ...
    const ret = await new UserApi().list();

    dataList.value = ret.data;
    pagination.total = ret.meta.total;
        ...
        FormRef.validate(async valid => {
          if (valid) {
            ...
            if (title === "新增") {
              // 实际开发先调用新增接口，再进行下面操作
              // 不使用邮箱的方式
              curData.email = curData.username + "@mail.com";
              const ret = await new UserApi().register(curData);
              chores();
            } else {
              // 实际开发先调用修改接口，再进行下面操作
              const ret = await new UserApi().update(curData);
              chores();
            }
          }
        });
